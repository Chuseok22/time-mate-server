...
- PR 제목 규칙:
  ...
  - 예시: `20251031_#30_서버_무중단_배포_설정 : feat : Blue/Green 무중단배포 도입 https://github.com/Chuseok22/meet-time-server/issues/30`
...
- linked issue 규칙:
  ...
  - 현재 패턴: `https://github.com/Chuseok22/meet-time-server/issues/[번호]`
...
- 배포 전 확인 사항:
  ...
  - `./gradlew test` — 전체 테스트 PASS (테스트 작성 시)
  - GitHub Actions PR 빌드 통과 확인 (time-mate-build.yml)

- feature flag 정책:
  - 현재 미사용

- rollback 필요 시 기준:
  - Blue/Green 배포 특성상 nginx 설정만 이전 포트로 되돌려 즉시 롤백 가능
  - `/actuator/health` 헬스체크 실패 시 배포 자동 중단
  - 치명적 버그 발생 시: 이전 커밋으로 revert PR 생성 후 재배포
  - nginx 설정 이슈 발생 시: NGINX_BACKUP_DIR 백업 파일로 복구
